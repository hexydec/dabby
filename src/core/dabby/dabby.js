    ...
    if (this instanceof dabby) {
        ...
        if (selector) {
            ...
            if (typeof selector === "string") {
                ...
                if (selector[0] !== "<") {
                    // normalise context
                    let obj = context ? $(context) : [document], i = obj.length;
                    while (i--) {
                ...
                nodes = Array.from(selector).filter((node, i, self) => self.indexOf(node) === i && ([1, 9, 11].includes(node.nodeType) || isWindow(node)));
            }
        }
