...
QUnit.test("$.ajax", function (assert) {
	...
	$.ajax({
		...
		success: function (response, status) {
			assert.equal(status, 200, "Can make an AJAX request with all settings as object");
			assert.ok(response.indexOf("Sample HTML File") !== -1, "AJAX request returned correct file");
			done();
		},
		complete: function (response, status) {
			assert.equal(status, 200, "Can set complete callback");
			assert.ok(response.indexOf("Sample HTML File") !== -1, "Complete callback returned correct file");
			done();
		},
		statusCode: {
			200: function (response, status) {
				assert.equal(status, 200, "Can set statusCode 200 callback");
	...
	$.ajax("../tests/assets/jsonp.js", {dataType: "jsonp", success: function (response, status) {
		assert.equal(status, 200, "Can include a javascript file via JSONP");
