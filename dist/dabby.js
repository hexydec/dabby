var $ = (function (exports) {
  ...
  $$1.isArray = arr => Array.isArray(arr);

  $$1.param = obj => {
    let params = [],
        add = (key, value, params) => {
      let isArr = $$1.isArray(value);
  ...
  $$1.ajax = (url, settings) => {
    ...
    if (sync || settings.crossDomain) {
      ...
      const callback = (xhr, status) => {
        let response = xhr.responseText; // parse JSON
      ...
      $$1.each(settings.headers, (key, value) => {
  ...
  ["get", "post"].forEach(name => {
    $$1[name] = (url, data, success, type) => {
      const isFunc = $$1.isFunction(data);
  ...
  $$1.getScript = (url, success) => $$1.ajax({
    url: url,
    dataType: "script",
    success: success
  });
    ...
    if (this[0]) {
      ...
      $$1.ajax(uri, {
        ...
        success: (response, status, xhr) => {
          ...
          let html = "",
              i = this.length; // refine by selector if supplied

          if (selector) {
            $$1(response).filter(selector).each((key, obj) => {
              html += obj.outerHTML;
            });
          } else {
            html = response;
          ...
          while (i--) {
            this[i].innerHTML = html; // fire success callback on nodes

            if (_success) {
              _success.call(this[i], response, status, xhr);
            }
          }
        }
      });
  ...
  $$1.fn.val = function (value) {
    ...
    if (value !== undefined) {
      ...
      while (i--) {
        let val = getVal(value, this[i], i, () => $$1(this[i]).val());

        if (this[i].multiple) {
          val = $$1.map($$1.isArray(val) ? val : [val], item => String(item)); // convert to string

          $$1("option", this[i]).each((key, obj) => {
      ...
      return this; // read value from first node
    } else if (this[0]) {
      // get multiple values
      if (this[0].multiple) {
        let values = [];
        $$1("option", this[0]).each((key, obj) => {
          if (obj.selected) {
  ...
  $$1.fn.serialize = function () {
    ...
    let params = {}; // process values

    obj.each((key, obj) => {
      const value = $$1(obj).val();
  ...
  ["on", "one", "off"].forEach(name => {
    $$1.fn[name] = function (events, selector, data, callback) {
      ...
      while (i--) {
        let e = events.length; // record the original function
          ...
          while (e--) {
            this[i].events.forEach((evt, n) => {
              ...
              if (index !== -1 && evt.callback === callback && evt.selector === selector) {
                this[i].removeEventListener(events[e], evt.func, {
                  once: evt.once,
  ...
  $$1.fn.attr = function (prop, value) {
    let isArr = $$1.isArray(prop),
        i,
        events$$1,
        arr = {}; // set properties

    if (isArr || value || value === null) {
      i = this.length; // normalise to array
  ...
  $$1.fn.data = function (name, data) {
    ...
    if (data !== undefined) {
      ...
      if (name === undefined) {
        let arr = {};
        $$1.each(this[0].dataset, (key, value) => {
          arr[key] = parse(value);
        });
  ...
  $$1.fn.trigger = function (name, data) {
    ...
    let i = this.length; // copy extra data to event object
  ...
  $$1.fn.has = function (selector) {
    return $$1(this.get().filter(node => !!$$1(selector, node).length));
  };
